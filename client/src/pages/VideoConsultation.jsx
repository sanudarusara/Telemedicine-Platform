...
const DOCTOR_SERVICE_URL =
  import.meta.env.VITE_DOCTOR_SERVICE_URL || "http://localhost:5001";

const TELEMEDICINE_SERVICE_URL =
  import.meta.env.VITE_TELEMEDICINE_SERVICE_URL || "http://localhost:5000";
...
const VideoConsultation = () => {
  ...
  const token = useMemo(() => getDoctorToken(), []);
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(`${DOCTOR_SERVICE_URL}/api/doctors/appointments`, {
        method: "GET",
        headers: {
  ...
  const startCall = async (appointment) => {
    try {
      ...
      const response = await fetch(
        `${TELEMEDICINE_SERVICE_URL}/api/video/create-room`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setActiveCall({
        appointmentId: appointment._id,
        patient: `Patient ${String(appointment.patientId).slice(-6)}`,
        date: dateTime.date,
        time: dateTime.time,
  ...
  const endCall = async () => {
    try {
      if (activeCall?.appointmentId) {
        await fetch(
          `${TELEMEDICINE_SERVICE_URL}/api/video/appointment/${activeCall.appointmentId}/end`,
          {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${token}`,
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-foreground">
                            Patient {String(c.patientId).slice(-6)}
                          </p>
