...
const NotificationsPopover = ({ userId, role }) => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/notifications-api/notifications?userId=${userId}&limit=20`);
        ...
        <div className="max-h-[450px] overflow-y-auto">
          ...
          {!loading && !error && notifications.length === 0 && (
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Appointment updates will appear here</p>
            </div>
          )}
          
          {!loading && !error && notifications.map((notification) => {
            const snapshot = notification.appointmentSnapshot;
            const isForPatient = role === 'patient';
            const otherParty = isForPatient ? snapshot?.doctorName : snapshot?.patientName;
                      ...
                      <div className="space-y-1 mt-1">
                        ...
                        <div className="flex items-center gap-1 text-xs">
                          <span className={`px-1.5 py-0.5 rounded-full text-xs ${
                            snapshot.status === 'confirmed' ? 'bg-green-100 text-green-700' :
                            snapshot.status === 'cancelled' ? 'bg-red-100 text-red-700' :
                            snapshot.status === 'pending' ? 'bg-yellow-100 text-yellow-700' :
