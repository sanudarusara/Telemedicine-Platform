...
(async ()=>{
  try {
    ...
    const dateQuery = process.argv[2] || '2026-04-25';
    // token
    ...
    const getPath = `/api/appointments/doctors/available-slots?doctorId=${doctorId}&date=${encodeURIComponent(dateQuery)}`;
    const getRes = await httpGet(getPath, { Authorization: 'Bearer ' + token });
    ...
    let slot = arr.find(s => (typeof s === 'object' && (s.available === true || s.isAvailable === true || s.status === 'available' || s.booked === false)) || (typeof s === 'string'));
    ...
    if (typeof slot === 'string') {
      ...
      date = slot.date || slot.day || slot.slotDate || slot.start || slot["date"] || dateQuery;
      timeSlot = slot.timeSlot || slot.slot || slot.startTime || slot.time || (slot.start ? new Date(slot.start).toLocaleTimeString() : undefined);
    }
    const appointment = { patientId, doctorId, date: date || '2026-04-25', timeSlot: timeSlot || '10:00 AM', consultationType: 'video', symptoms: 'Cardiac checkup', notes: 'Booked via script using appointment-service slots', paymentAmount: slot.fee || 1500 };
